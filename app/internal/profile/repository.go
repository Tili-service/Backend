...
func (r *Repository) FindByStoreAndPin(ctx context.Context, storeID uuid.UUID, pin string) (*Profile, error) {
	p := new(Profile)
...
func (r *Repository) DeleteByStoreID(ctx context.Context, storeID uuid.UUID) error {
	_, err := r.db.NewDelete().Model(&Profile{}).Where("store_id = ?", storeID).Exec(ctx)
	return err
}

func (r *Repository) PinExistsInStore(ctx context.Context, storeID uuid.UUID, pin string) (bool, error) {
...
func (r *Repository) FindAllProfilesByStoreID(ctx context.Context, storeID uuid.UUID) ([]*Profile, error) {
