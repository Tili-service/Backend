...
func TestRepository_FindByID(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta(`SELECT "s"."store_id", "s"."name", "s"."buyer_id", "s"."licence_id", "s"."date_creation", "s"."numero_tva", "s"."sumup_merchant_code", "s"."sumup_access_token", "s"."sumup_refresh_token", "s"."siret" FROM "store" AS "s" WHERE (store_id = `)).
		WillReturnRows(rows)
...
func TestRepository_FindAll(t *testing.T) {
	...
	mock.ExpectQuery(`^SELECT "s"\."store_id", "s"\."name", "s"\."buyer_id", "s"\."licence_id", "s"\."date_creation", "s"\."numero_tva", "s"\."sumup_merchant_code", "s"\."sumup_access_token", "s"\."sumup_refresh_token", "s"\."siret" FROM "store" AS "s"$`).WillReturnRows(rows)
...
func TestRepository_FindByLicenceID(t *testing.T) {
	...
	licenceID := uuid.New()
	rows := sqlmock.NewRows([]string{"store_id", "name", "buyer_id", "licence_id", "date_creation", "numero_tva", "sumup_merchant_code", "sumup_access_token", "sumup_refresh_token", "siret"}).AddRow(uuid.New(), "Licence Store", uuid.New(), licenceID, time.Now(), "", "", "", "", "")
